	...
	if (mlevel >= level) {
		...
		if (mgd != NULL) {
			midgard_error_default_log(
					domain, level, msg, MIDGARD_IS_CONNECTION(mgd) ? MIDGARD_CONNECTION(mgd) : NULL);
		}
	}
...
static zend_bool php_midgard_initialize_configs()
{
	...
	while (files[i] != NULL) {
		...
		if (!midgard_config_read_file(config, files[i], FALSE, &err)) {
			php_error(E_WARNING, "Failed to read %s config file. %s", files[i], err->message ? err->message : "");
...
PHP_MINIT_FUNCTION(midgard2)
{
	...
	if (ze != NULL) {
		php_error(E_NOTICE, "Module %s already loaded. It's recommended to load it via php.ini", MIDGARD_PACKAGE_NAME);
		return SUCCESS;
	}

	global_loghandler = g_log_set_handler(G_LOG_DOMAIN, G_LOG_LEVEL_MASK, midgard_error_default_log, NULL);
	...
	/* Initialize handlers */
	memcpy(&php_midgard_gobject_handlers, zend_get_std_object_handlers(), sizeof(zend_object_handlers));
	...
	ce_midgard_error_exception = zend_register_internal_class_ex(&ce, zend_exception_get_default(TSRMLS_C), NULL TSRMLS_CC);
...
#define MGD_PHP_REGISTER_CONSTANT(name) REGISTER_LONG_CONSTANT(#name, name, CONST_CS | CONST_PERSISTENT)
...
#undef MGD_PHP_REGISTER_CONSTANT
	...
	if (MGDG(superglobals_compat)) {
		...
		zend_register_auto_global("_MIDGARD",            sizeof("_MIDGARD")-1,            NULL TSRMLS_CC);
		zend_register_auto_global("_MIDCOM",             sizeof("_MIDCOM")-1,             NULL TSRMLS_CC);
		zend_register_auto_global("_MIDGARD_CONNECTION", sizeof("_MIDGARD_CONNECTION")-1, NULL TSRMLS_CC);
	}
...
PHP_RINIT_FUNCTION(midgard2)
{
	...
	if (php_midgard_is_http_env()) {
		...
		zval *instance;
		zend_call_method_with_0_params(NULL, php_midgard_connection_class, NULL, "get_instance", &instance);
...
PHP_RSHUTDOWN_FUNCTION(midgard2)
{
	...
	if (MGDG(midgard_memory_debug)) {
		...
		int rv = zend_hash_find(&module_registry, MIDGARD_PACKAGE_NAME, strlen(MIDGARD_PACKAGE_NAME) + 1, (void**)&module);
		if (rv == SUCCESS) {
...
MGD_FUNCTION(ret_type, is_guid, (type param))
{
	...
	if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS() TSRMLS_CC, "s", &guid, &guidl) == FAILURE) {
		RETURN_FALSE;
	}
