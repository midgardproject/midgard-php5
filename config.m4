...
if test "$PHP_MIDGARD2" != "no"; then
    ...
    if test -f "$PKG_CONFIG"; then
        if $PKG_CONFIG --atleast-version=10.05.99 midgard3; then
            MIDGARD2_INCLINE=`$PKG_CONFIG --cflags midgard3`
            MIDGARD2_LIBLINE=`$PKG_CONFIG --libs midgard3`
        else
            AC_MSG_ERROR([Midgard3 version 10.05.99 or greater required.])
        fi
        ...
        PHP_NEW_EXTENSION(midgard2, \
            ...
            php_midgard_query.c php_midgard_query_storage.c php_midgard_query_holders.c php_midgard_query_constraints.c php_midgard_query_executors.c \
            , $ext_shared)
    else
