...
if test "$PHP_MIDGARD2" != "no"; then
    ...
    if test -f "$PKG_CONFIG"; then
        if $PKG_CONFIG --atleast-version=9.09.99 midgard2; then
            MIDGARD2_INCLINE=`$PKG_CONFIG --cflags midgard2`
            MIDGARD2_LIBLINE=`$PKG_CONFIG --libs midgard2`
        else
            AC_MSG_ERROR([Midgard2 version 9.09.99 or greater required.])
        fi
        ...
        PHP_NEW_EXTENSION(midgard2, \
            midgard.c preparser.c query_builder.c \
            php_midgard_reflection_property.c php_midgard_collector.c php_midgard_object_parameter.c php_midgard_object_attachment.c \
            php_midgard_config.c php_midgard_gobject_generic.c php_midgard_user.c php_midgard_blob.c php_midgard_object_class.c \
            php_midgard_compat.c php_midgard_connection.c php_midgard_object.c php_midgard_replicator.c php_midgard_dbus.c \
