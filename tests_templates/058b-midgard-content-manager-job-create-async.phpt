...
$job2 = $manager->create_job(MIDGARD_CONTENT_MANAGER_JOB_CREATE, $content2, $reference);
...
var_dump($job->is_running());
var_dump($job2->is_running());
var_dump($content->guid);
var_dump($content2->guid);
var_dump(strlen($content->guid) > 1);
var_dump(strlen($content2->guid) > 1);
var_dump($content->name);
var_dump($content2->name);
$content->purge(false);
